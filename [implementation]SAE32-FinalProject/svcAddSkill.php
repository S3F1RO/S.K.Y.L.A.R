...
$mainName = NULL;
if (preg_match("/^[A-Za-z0-9\-éèêëÉÈÊËïÏàÀçÇ&\' ]{1,20}$/", $data['mainName'])) $mainName = $db->real_escape_string($data['mainName']);
$subName = "";
if (preg_match("/^[A-Za-z0-9\-éèêëïàç&\' ]{1,20}$/", $data['subName'])) $subName = $db->real_escape_string($data['subName']);
$domain = NULL;
if (preg_match("/^[A-Za-z0-9\-éèêëïàç&\' ]{1,20}$/", $data['domain'])) $domain = $db->real_escape_string($data['domain']);
$level = NULL;
